...
def fun(file):
    ...
    for t in range(100):
        ...
        n_feature = int(math.sqrt(train.shape[1] - 4))
        ...
        clf = GradientBoostingClassifier(max_depth=best_max_depth, max_features=best_max_features,
        ...
        def hyperopt_model_score_GBM_remove(params):
            clf = GradientBoostingClassifier(**params)
        ...
        def fn_GBM_remove(params):
            acc = hyperopt_model_score_GBM_remove(params)
            return -acc

        trials = Trials()

        best = fmin(
            fn=fn_GBM_remove, space=space_GBM_remove, algo=atpe.suggest, max_evals=100, trials=trials)
        print("Best: {}".format(best))
        ...
        clf1 = GradientBoostingClassifier(max_depth=best_max_depth, max_features=best_max_features,
                                         n_estimators=best_n_estimators)
    ...
    ss='C:/gln/myclassoverlap/results/feature/GBM/original/orighnal_'+file
    feature_original.to_csv(ss,index=False)
    ss1='C:/gln/myclassoverlap/results/feature/GBM/remove/remove_'+file
