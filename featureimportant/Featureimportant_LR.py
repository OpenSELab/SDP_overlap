...
def fun(file):
    ...
    for t in range(100):
        ...
        def hyperopt_model_score_LR_remove(params):
            clf = LogisticRegression(**params)
        ...
        def fn_LR_remove(params):
            acc = hyperopt_model_score_LR_remove(params)
            return -acc

        trials = Trials()

        best = fmin(
            fn=fn_LR_remove, space=space_LR_remove, algo=atpe.suggest, max_evals=100, trials=trials)
        print("Best: {}".format(best))
        if (best['penalty'] == 0):
            penalty = "l1"
        else:
        ...
        clf1 = LogisticRegression(penalty=penalty, C=best_C)
    ...
    ss='C:/gln/myclassoverlap/results/featureimportant/new/LR/original/orighnal_'+file
    feature_original.to_csv(ss,index=False)
    ss1='C:/gln/myclassoverlap/results/featureimportant/new/LR/remove/remove_'+file
