...
def fun(file):
    ...
    for t in range(100):
        ...
        train_separating=train[train["laplabel"] == 1]
        ...
        clf= RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
        ...
        def hyperopt_model_score_RF_remove(params):
            clf = RandomForestClassifier(**params, n_jobs=5)
        ...
        def fn_RF_remove(params):
            acc = hyperopt_model_score_RF_remove(params)
            return -acc

        trials = Trials()

        best = fmin(
            fn=fn_RF_remove, space=space_RF_remove, algo=atpe.suggest, max_evals=100, trials=trials)
        print("Best: {}".format(best))

        best_max_depth = best['max_depth']
        best_n_estimators = best['n_estimators']

        clf1 = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
    ...
    ss='C:/gln/myclassoverlap/results/featureimportant/new/RF/original/orighnal_'+file
    feature_original.to_csv(ss,index=False)
    ss1='C:/gln/myclassoverlap/results/featureimportant/new/RF/remove/remove_'+file
