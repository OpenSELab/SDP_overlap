...
if __name__ == '__main__':
    file = ['prop-1-92.csv','prop-2-256.csv','prop-1-164.csv']#'prop-3-318.csv','prop-4-355.csv', 'prop-5-4.csv', 'prop-5-40.csv', 'prop-5-85.csv','prop-5-121.csv','prop-5-157.csv',
    min_max_scaler_original = MinMaxScaler()#'prop-1-44.csv','prop-5-185.csv','xalan-2.5.csv','xalan-2.6.csv'
 ...
 #           print(x_train_KNN.shape[1])
          ...
          #  rf_KNN.predict(x_test_KNN)
            ...
            def hyperopt_model_score_SVM_lap(params):
                clf = SVC(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_SVM_lap(params):
                acc = hyperopt_model_score_SVM_lap(params)
                return -acc
            ...
            best = fmin(
                fn=fn_SVM_lap, space=space_SVM_lap, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['kernel'] == 0):
                kernel = 'linear'
            else:
            ...
            rf_overlap = SVC(C=best['C'], kernel=kernel, gamma=best['gamma'], probability=True)
            ...
            if (x_test_overlap.shape[0] > 0):
                ...
                def hyperopt_model_score_SVM_laplap(params):
                    clf = SVC(**params)
                    return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
                ...
                def fn_SVM_laplap(params):
                    acc = hyperopt_model_score_SVM_laplap(params)
                    return -acc
                ...
                best = fmin(
                    fn=fn_SVM_laplap, space=space_SVM_laplap, algo=atpe.suggest, max_evals=100, trials=trials)
                print("Best: {}".format(best))
                if (best['kernel'] == 0):
                    kernel = 'linear'
                else:
                ...
                rf_laplap = SVC(C=best['C'], kernel=kernel, gamma=best['gamma'], probability=True)
              #
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/performance/SVM/" + file[i]
