...
if __name__ == '__main__':
    file = ['prop-5-157.csv','prop-5-185.csv','xalan-2.5.csv','xalan-2.6.csv']#'prop-1-44.csv',
    min_max_scaler_original = MinMaxScaler()#'camel-1.2.csv','derby-10.2.1.6.csv','derby-10.3.1.4.csv','eclipse34_debug.csv','prop-1-92.csv','prop-1-164.csv','prop-2-256.csv','prop-4-355.csv', 'prop-5-4.csv', 'prop-5-40.csv', 'prop-5-85.csv', 'prop-5-121.csv',
 ...
 #           print(x_train_KNN.shape[1])
            ...
            x_test_KNN = min_max_scaler_KNN.fit_transform(x_test)
          ...
          #  rf_KNN.predict(x_test_KNN)
            ...
            def hyperopt_model_score_LR_lap(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_lap(params):
                acc = hyperopt_model_score_LR_lap(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_lap, space=space_LR_lap, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_overlap = LogisticRegression(penalty=penalty, C=best_C)
            ...
            if (x_test_overlap.shape[0] > 0):
                ...
                def hyperopt_model_score_LR_laplap(params):
                    clf = LogisticRegression(**params)
                    return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
                ...
                def fn_LR_laplap(params):
                    acc = hyperopt_model_score_LR_laplap(params)
                    return -acc
                ...
                best = fmin(
                    fn=fn_LR_laplap, space=space_LR_laplap, algo=atpe.suggest, max_evals=100, trials=trials)
                print("Best: {}".format(best))
                if (best['penalty'] == 0):
                    penalty = "l1"
                else:
                ...
                rf_laplap = LogisticRegression(penalty=penalty, C=best_C)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/performance/LR/" + file[i]
