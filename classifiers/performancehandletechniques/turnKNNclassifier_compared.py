...
if __name__ == '__main__':
 ...
 #           print(x_train_KNN.shape[1])
          ...
          #  rf_KNN.predict(x_test_KNN)
            ...
            def hyperopt_model_score_KNN_lap(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_lap(params):
                acc = hyperopt_model_score_KNN_lap(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_lap, space=space_KNN_lap, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_overlap = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            if (x_test_overlap.shape[0] > 0):
                ...
                def hyperopt_model_score_KNN_laplap(params):
                    clf = KNeighborsClassifier(**params)
                    return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
                ...
                def fn_KNN_laplap(params):
                    acc = hyperopt_model_score_KNN_laplap(params)
                    return -acc
                ...
                best = fmin(
                    fn=fn_KNN_laplap, space=space_KNN_laplap, algo=atpe.suggest, max_evals=100, trials=trials)
                print("Best: {}".format(best))

                best_n_neighbors = best['n_neighbors']

                rf_laplap = KNeighborsClassifier(n_neighbors=best_n_neighbors)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/performance/KNN/" + file[i]
