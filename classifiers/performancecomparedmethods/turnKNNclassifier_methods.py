       ...
       #     print(y_train_SVDD)
         ...
         #   print(y_train_combine)
           ...
           # n_train_defect_grid = train_grid[train_grid['label'] == 1].shape[0]
            ...
            rf_original = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            def hyperopt_model_score_KNN_KNN(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_KNN(params):
                acc = hyperopt_model_score_KNN_KNN(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_KNN, space=space_KNN_KNN, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_KNN = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            def hyperopt_model_score_KNN_Kmeans(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_Kmeans(params):
                acc = hyperopt_model_score_KNN_Kmeans(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_Kmeans, space=space_KNN_Kmeans, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_Kmeans = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            def hyperopt_model_score_KNN_SMR(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_SMR(params):
                acc = hyperopt_model_score_KNN_SMR(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_SMR, space=space_KNN_SMR, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_SMR = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            def hyperopt_model_score_KNN_SVDD(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_SVDD(params):
                acc = hyperopt_model_score_KNN_SVDD(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_SVDD, space=space_KNN_SVDD, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_SVDD = KNeighborsClassifier(n_neighbors=best_n_neighbors)
            ...
            def hyperopt_model_score_KNN_combine(params):
                clf = KNeighborsClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_KNN_combine(params):
                acc = hyperopt_model_score_KNN_combine(params)
                return -acc
            ...
            best = fmin(
                fn=fn_KNN_combine, space=space_KNN_combine, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_n_neighbors = best['n_neighbors']

            rf_combine = KNeighborsClassifier(n_neighbors=best_n_neighbors)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/compared/KNN/" + file[i]
