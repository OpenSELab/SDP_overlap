       ...
       #     print(y_train_SVDD)
         ...
         #   print(y_train_combine)
           ...
           # n_train_defect_grid = train_grid[train_grid['label'] == 1].shape[0]
            ...
            best = fmin(
                fn=fn_LR, space=space_LR, algo=atpe.suggest, max_evals=100, trials=trials)
            ...
            rf_original = LogisticRegression(penalty=penalty, C=best_C)
            ...
            def hyperopt_model_score_LR_KNN(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_KNN(params):
                acc = hyperopt_model_score_LR_KNN(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_KNN, space=space_LR_KNN, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_KNN = LogisticRegression(penalty=penalty, C=best_C)
            ...
            def hyperopt_model_score_LR_Kmeans(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_Kmeans(params):
                acc = hyperopt_model_score_LR_Kmeans(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_Kmeans, space=space_LR_Kmeans, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_Kmeans = LogisticRegression(penalty=penalty, C=best_C)
            ...
            def hyperopt_model_score_LR_SMR(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_SMR(params):
                acc = hyperopt_model_score_LR_SMR(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_SMR, space=space_LR_SMR, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_SMR = LogisticRegression(penalty=penalty, C=best_C)
            ...
            def hyperopt_model_score_LR_SVDD(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_SVDD(params):
                acc = hyperopt_model_score_LR_SVDD(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_SVDD, space=space_LR_SVDD, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_SVDD = LogisticRegression(penalty=penalty, C=best_C)
            ...
            def hyperopt_model_score_LR_combine(params):
                clf = LogisticRegression(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_LR_combine(params):
                acc = hyperopt_model_score_LR_combine(params)
                return -acc
            ...
            best = fmin(
                fn=fn_LR_combine, space=space_LR_combine, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            if (best['penalty'] == 0):
                penalty = "l1"
            else:
            ...
            rf_combine = LogisticRegression(penalty=penalty, C=best_C)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/compared/LR/" + file[i]
