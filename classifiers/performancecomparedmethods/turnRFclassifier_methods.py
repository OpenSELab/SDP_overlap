...
if __name__ == '__main__':
    ...
    for i in range(len(file)):
        s = 'D:/gln/myclassoverlap/Data/new/gooddata/data/' + file[i]
        g = os.walk(s)
        trials = Trials()
     ...
     #       print(y_train_original)
      ...
      #      print(y_train_KNN)
       ...
       #     print(y_train_SVDD)
            ...
            # train_grid = train[train['grid'] == 0]
            # x_train_grid  = train_grid .iloc[:, :train_grid .shape[1] - 21].values
            # y_train_grid  = train_grid .iloc[:, train_grid .shape[1] - 20].values
            # print(y_train_grid )
            # x_train_grid  = min_max_scaler_grid .fit_transform(x_train_grid )
            # x_test_grid  = min_max_scaler_grid .transform(x_test)
         ...
         #   print(y_train_combine)
           ...
           # n_train_defect_grid = train_grid[train_grid['label'] == 1].shape[0]
            ...
            best = fmin(
                fn=fn_RF, space=space_RF, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_original = RandomForestClassifier( max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            def hyperopt_model_score_RF_KNN(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_KNN(params):
                acc = hyperopt_model_score_RF_KNN(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_KNN, space=space_RF_KNN, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_KNN = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            def hyperopt_model_score_RF_Kmeans(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_Kmeans(params):
                acc = hyperopt_model_score_RF_Kmeans(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_Kmeans, space=space_RF_Kmeans, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_Kmeans = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            def hyperopt_model_score_RF_SMR(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_SMR(params):
                acc = hyperopt_model_score_RF_SMR(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_SMR, space=space_RF_SMR, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_SMR = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            def hyperopt_model_score_RF_SVDD(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_SVDD(params):
                acc = hyperopt_model_score_RF_SVDD(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_SVDD, space=space_RF_SVDD, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_SVDD = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            def hyperopt_model_score_RF_combine(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_combine(params):
                acc = hyperopt_model_score_RF_combine(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_combine, space=space_RF_combine, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_combine = RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
        ...
        data_RF.columns = ['auc_RF_original', 'recall_RF_original', 'brier_RF_original', 'pop_RF_original', 'pf_RF_original', 'auc_RF_KNN', 'recall_RF_KNN', 'brier_RF_KNN', 'pop_RF_KNN', 'pf_RF_KNN','auc_RF_Kmeans', 'recall_RF_Kmeans', 'brier_RF_Kmeans', 'pop_RF_Kmeans', 'pf_RF_Kmeans','auc_RF_SMR', 'recall_RF_SMR', 'brier_RF_SMR', 'pop_RF_SMR', 'pf_RF_SMR','auc_RF_SVDD', 'recall_RF_SVDD', 'brier_RF_SVDD', 'pop_RF_SVDD', 'pf_RF_SVDD','auc_RF_grid', 'recall_RF_grid', 'brier_RF_grid', 'pop_RF_grid', 'pf_RF_grid','auc_RF_combine', 'recall_RF_combine', 'brier_RF_combine', 'pop_RF_combine', 'pf_RF_combine']#,
        s3 = "D:/gln/myclassoverlap/results/compared/RF/" + file[i]
