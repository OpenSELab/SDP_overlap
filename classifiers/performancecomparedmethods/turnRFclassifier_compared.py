...
if __name__ == '__main__':
 ...
 #           print(x_train_KNN.shape[1])
            ...
            def hyperopt_model_score_RF_lap(params):
                clf = RandomForestClassifier(**params, n_jobs=5)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_RF_lap(params):
                acc = hyperopt_model_score_RF_lap(params)
                return -acc
            ...
            best = fmin(
                fn=fn_RF_lap, space=space_RF_lap, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))

            best_max_depth = best['max_depth']
            best_n_estimators = best['n_estimators']

            rf_overlap= RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
            ...
            if (x_test_overlap.shape[0] > 0):
                ...
                def hyperopt_model_score_RF_laplap(params):
                    clf = RandomForestClassifier(**params, n_jobs=5)
                    return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
                ...
                def fn_RF_laplap(params):
                    acc = hyperopt_model_score_RF_laplap(params)
                    return -acc
                ...
                best = fmin(
                    fn=fn_RF_laplap, space=space_RF_laplap, algo=atpe.suggest, max_evals=100, trials=trials)
                print("Best: {}".format(best))

                best_max_depth = best['max_depth']
                best_n_estimators = best['n_estimators']

                rf_laplap= RandomForestClassifier(max_depth=best_max_depth, n_estimators=best_n_estimators)
                rf_laplap.fit(x_test_overlap,y_test_overlap)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/performance/RF/" + file[i]
