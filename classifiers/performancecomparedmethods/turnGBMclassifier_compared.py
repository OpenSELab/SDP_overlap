...
if __name__ == '__main__':
    file = ['derby-10.3.1.4.csv','eclipse34_debug.csv','prop-1-44.csv','prop-1-92.csv','prop-1-164.csv','prop-2-256.csv','prop-3-318.csv','prop-4-355.csv','prop-5-4.csv','prop-5-40.csv','prop-5-85.csv','prop-5-121.csv','prop-5-157.csv','prop-5-185.csv','xalan-2.5.csv','xalan-2.6.csv']
    min_max_scaler_original = MinMaxScaler()#'camel-1.2.csv','derby-10.2.1.6.csv',
 ...
 #           print(x_train_KNN.shape[1])
            ...
            rf_original =GradientBoostingClassifier(max_depth=best_max_depth,max_features=best_max_features,n_estimators=best_n_estimators)
          ...
          #  rf_KNN.predict(x_test_KNN)
            ...
            def hyperopt_model_score_GBM_lap(params):
                clf = GradientBoostingClassifier(**params)
                return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
            ...
            def fn_GBM_lap(params):
                acc = hyperopt_model_score_GBM_lap(params)
                return -acc
            ...
            best = fmin(
                fn=fn_GBM_lap, space=space_GBM_lap, algo=atpe.suggest, max_evals=100, trials=trials)
            print("Best: {}".format(best))
            ...
            rf_overlap = GradientBoostingClassifier(max_depth=best_max_depth, max_features=best_max_features,
                                                n_estimators=best_n_estimators)
            ...
            if (x_test_overlap.shape[0] > 0):
                ...
                def hyperopt_model_score_GBM_laplap(params):
                    clf = GradientBoostingClassifier(**params)
                    return cross_val_score(clf, x_train_original, y_train_original, scoring="roc_auc", cv=2).mean()
                ...
                def fn_GBM_laplap(params):
                    acc = hyperopt_model_score_GBM_laplap(params)
                    return -acc
                ...
                best = fmin(
                    fn=fn_GBM_laplap, space=space_GBM_laplap, algo=atpe.suggest, max_evals=100, trials=trials)
                print("Best: {}".format(best))
                ...
                rf_laplap = GradientBoostingClassifier(max_depth=best_max_depth, max_features=best_max_features,
                                                        n_estimators=best_n_estimators)
        ...
        s3 = "E:/gln/C/myclassoverlap/rebuttal/results/performance/GBM1/" + file[i]
